...
const (
	// 1000 clients do transfer, withdrawal, deposit
	// each action 5 times in 10 ATMs.
	// Transfer recevier and atm location are random.
	TEST_VOL = 1000 // for sub 1 at least ( >= 2)
	ATM_VOL  = 10
)
...
func sub_1() error {
	...
	oneATM, wg := atmlist[0], sync.WaitGroup{}
	datas := sub_1_data()
	for _, data := range datas {
		...
		for i := 0; i < 1000; i++ {
			...
			go func(from, to *bank.Account) {
				defer wg.Done()
				oneATM.Read(from, PWD)
				defer oneATM.Return()
				if err := oneATM.Transfer(to.Number, 10); err != nil {
					logger.Println(from.Number, err)
					return
				}
	...
	oneATM.Return()
	// Money check
	if !((datas[0].FromAccount.Balance + datas[0].ToAccount.Balance) == 2000) {
